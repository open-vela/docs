...
4. 组委会向你提供一条 `repo init -u <manifest 仓库地址> -b dev-ai-contest-2026 --git-lfs` 命令（该 manifest 仓库内的清单文件已登记好「openvela 全量源码 + 你的专属仓库」），执行后再 `repo sync` 即可一键拉取完整工程；同时把你的 GitHub 账号加为仓库协作者（开通 push 权限）。
