...
5. **拉取工程**：用组委会提供的 `repo init` + `repo sync` 一键拉取「openvela 全量源码 + 你的专属仓」。**注意：`repo init` 需带 `--git-lfs` 参数**（否则含 Git LFS 大文件的预编译库无法正确拉取，编译时可能报 `libgui_wrapper.a` 等链接失败），完整命令详见 [《参赛代码提交指南》](./code_submission_guide.md)。
